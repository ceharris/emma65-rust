      ...
      {rows === null || displayRows === null ? (
        ...
        <div className="symbols-table">
          <div className="symbols-header-row">
            <span className="symbols-col-name sortable" onClick={() => handleHeaderClick("name")}>Name{sortIndicator("name")}</span>
            <span className="symbols-col-address sortable" onClick={() => handleHeaderClick("address")}>Address{sortIndicator("address")}</span>
            <span className="symbols-col-source sortable" onClick={() => handleHeaderClick("source")}>Source{sortIndicator("source")}</span>
          ...
          <div className="symbols-body">
            {displayRows.map((row, index) => (
              <div key={`${row.name}-${row.source}-${index}`} className="symbols-row">
                <span className="symbols-col-name" title={row.name}>{row.name}</span>
                <span className="symbols-col-address">{formatAddr(row.address)}</span>
                <span className="symbols-col-source" title={row.source_path ?? row.source}>{row.source}</span>
