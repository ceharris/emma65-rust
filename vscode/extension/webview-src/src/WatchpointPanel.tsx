...
export default function WatchpointPanel() {
  ...
  // Unlike the Tauri debugger's WatchpointPanel, there's no `ExecState` prop to gate
  // on here (the vscode extension doesn't track a running/stopped state outside the
  // DAP session itself). Editing is only blocked by a whole-file compile error;
  // attempting an add/edit/remove/toggle while the CPU is running instead surfaces
  // "CPU not ready" through the same error dialog already used for compile errors.
  const canEdit = snapshot?.compileError == null;

  const fetchWatchpoints = useCallback(async () => {
    try {
      const result = await invoke<WatchpointsSnapshot>("get");
      setSnapshot(result);
      ...
      console.error("get watchpoints failed:", e);
    }
  }, []);
  ...
  // Re-fetch on every DAP `stopped` event, the closest equivalent of the Tauri
  // debugger's `debugger-halted`/`debugger-run-stopped` events (see `watchpoints.ts`'s
  // matching comment, ported from `trace.ts`'s identical live-follow pattern).
  useEffect(() => {
    return listen("halted", () => { fetchWatchpoints(); });
  }, [fetchWatchpoints]);
  ...
  const removeWatchpointAt = useCallback(async (index: number) => {
    try {
      const result = await invoke<WatchpointsSnapshot>("remove", { index });
      setSnapshot(result);
      setSelectedIndex(null);
      setExpandedIndex(null);
    } catch (e) {
      console.error("remove watchpoint failed:", e);
    }
  }, []);
  ...
  const toggleWatchpointAt = useCallback(async (index: number) => {
    try {
      const result = await invoke<WatchpointsSnapshot>("toggle", { index });
      setSnapshot(result);
    } catch (e) {
      console.error("toggle watchpoint failed:", e);
    }
  }, []);

  /** Delete key removes the selected watchpoint. */
  useEffect(() => {
  ...
  /** Validates the input, invokes edit, and closes the popover on success. */
  const commitEditWatchpoint = useCallback(async () => {
    ...
    try {
      const result = await invoke<WatchpointsSnapshot>("edit", { index: editDialog.index, source });
      setSnapshot(result);
      setEditDialog(null);
    } catch (e) {
  ...
  /** Validates the input, invokes add, and closes the popover on success. */
  const commitAddWatchpoint = useCallback(async () => {
    ...
    try {
      const result = await invoke<WatchpointsSnapshot>("add", { source });
      setSnapshot(result);
      setAddDialog(null);
    } catch (e) {
    ...
    <div className="watchpoint-panel" ref={panelRef}>
      <div className="watchpoint-header">
        ...
        <button
          ...
          disabled={!canEdit}
          title={canEdit ? "Add watchpoint" : "Fix the watchpoints.emw compile error to edit watchpoints"}
        >
          +
        </button>
      </div>
      ...
      ) : snapshot.compileError !== null ? (
        <div className="watchpoint-error">{snapshot.compileError}</div>
            ...
            return (
              ...
              >
                <span
                  ...
                  title={canEdit ? (row.enabled ? "Disable watchpoint" : "Enable watchpoint") : "Fix the watchpoints.emw compile error to edit watchpoints"}
                >
                  {row.enabled ? "●" : "⊘"}
                </span>
                <span
                ...
                <button
                  ...
                  disabled={!canEdit}
                  title={canEdit ? "Remove watchpoint" : "Fix the watchpoints.emw compile error to edit watchpoints"}
                >
                  ×
                </button>
              </div>
      ...
      {snapshot !== null && snapshot.compileError === null && (
