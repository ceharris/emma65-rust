...
**Detach-to-window: custom-built, NOT dockview's own popout feature.** Every docking library's popout is `window.open()`-based, which in Tauri 2 produces a webview with no Tauri window label — `get_webview_window`/`emit_to` can't address it, `capabilities/default.json`'s label-scoped permissions don't cover it, it doesn't get the native menu treatment, and it doesn't participate in the close-lifecycle hooks the project already relies on (`install_toggleable_window_lifecycle`, `request_exit`). The project has already shipped the right mechanism for this — `terminal.rs`'s `TERMINAL_WINDOW_LABEL` + a `tauri.conf.json`-declared window + `emit_to` targeting — Phase 6 generalizes it to a **statically declared, initially hidden window** (`terminal-detached`), shown/hidden on each detach cycle exactly like Terminal/Trace/Log are today, rather than dynamically built/destroyed via `WebviewWindowBuilder`. (Originally planned as dynamic construction; revised after Phase 0's spike found that building a window with `WebviewWindowBuilder` freezes the entire app for the duration of construction — see Phase 0 findings, finding 4. A static window still has one fixed label, so this still sidesteps any need for Tauri 2's glob-pattern capability matching.) `TerminalPanel` is still removed from the dock on detach and re-added on reattach, so the fresh-mount-per-cycle behavior (and the accepted scrollback loss below) is unchanged.
...
#### Phase 0 findings (2026-08-11, PR #386)

Prototype: hidden `dockview-spike` window (`DockviewSpike.tsx`, `spike.rs`) hosting a `dockview-react` shell with a real `StackPanel` tab, a placeholder tab, and a bare xterm tab; a dynamically built `stack-detached` window (`WebviewWindowBuilder`, fixed reused label) for the detach mechanism; and, added mid-UAT to chase down finding 4 below, a second detach path using a statically declared, initially hidden `stack-detached-static` window merely shown/hidden. All UAT performed by the user under `cargo tauri dev` on WebKitGTK.

1. **Dockview confirmed as the library.** Drag/resize/theming render correctly under WebKitGTK in both light and dark. Theming is just a `DockviewTheme` object (`name`/`className`/`colorScheme`) plus `--dv-*` CSS custom properties scoped under that class — no shipped theme needed; `styles/spike.scss` maps a working subset directly onto the existing `--color-*` palette. Every own-document dockview window still needs the same `get_theme`/`theme-changed`/`matchMedia` theme-sync duplication `TerminalWindow`/`LogPanel`/`TracePanel` already carry (missed on the spike's first pass, confirmed by UAT, fixed via `SpikeThemeSync.tsx`) — Phase 2 must not forget it for the main dockview shell either, even though the main window currently gets theme for free via `ThemeProvider`/React context (context still won't reach a Phase 6 detached window).
2. **Critical question resolved: dockview does NOT unmount an inactive tab's panel.** Confirmed both by source inspection and live UAT. `dockview-react` mounts each panel's React tree exactly once via `ReactDOM.createPortal` into a container div created when the panel is added (`ReactPart` in `dockview-react`'s bundle); the default `onlyWhenVisible` renderer only reparents/removes that *container* from the document on tab-switch (`dockviewComponent`'s `renderPanel`/`closePanel`), it never disposes the portal. Live confirmation: `SpikeMountLog` only logged mount/unmount around initial `addPanel` calls (React 18 StrictMode's dev-only double-invoke of a fresh mount) and around real `removePanel`/`addPanel` calls (detach/reattach) — never merely from switching tabs. **This removes the biggest risk flagged for Phase 5's `terminal_ready` handshake**: `TerminalPanel` will mount once when the dock layout is built, regardless of which tab in its group is initially active, so the handshake doesn't need the eager-mount-every-panel or buffer-until-listener-attaches mitigations the plan held in reserve.
3. **xterm refit confirmed.** `ResizeObserver`-driven `fitAddon.fit()` correctly refits on a dockview split drag and on an inactive-to-active tab transition.
4. **Detach mechanism works, but its construction strategy needs to change.** `StackPanel`'s `invoke()`-backed commands function identically in a dynamically created `WebviewWindowBuilder` window — no capability or wiring problems. But building that window **freezes the entire app** (every window's panel updates stop) for the duration of `WebviewWindowBuilder::build()`, reproducible on every single detach; UAT also surfaced a `[TAURI] Couldn't find callback id …` warning correlated with the same event, also on every detach. Reattach (`window.destroy()`) causes neither symptom, ruling out "any window lifecycle op" and pinning it specifically to webview *construction*. An A/B test added mid-spike — a statically declared, initially hidden `stack-detached-static` window (mirroring Terminal/Trace/Log's existing pattern) merely shown/hidden instead of built/destroyed — produced no freeze, and the orphaned-callback warning dropped from every cycle to occasional. **Recommendation: Phase 6 should not build/destroy a `WebviewWindowBuilder` window on every detach cycle.** Pre-declare `terminal-detached` in `tauri.conf.json` as a hidden static window at startup (like Terminal/Trace/Log today) and use `show()`/`hide()` for detach/attach, while still removing/re-adding the `TerminalPanel` dock panel component on each cycle to keep the already-accepted fresh-mount/scrollback-loss behavior. This doesn't reopen the glob-pattern-capabilities question the original dynamic design was chosen to avoid — a static window still has one fixed label, so a plain `capabilities/default.json` entry is still sufficient. The "Key decisions" section above and Phase 6's design need to be revised accordingly before that issue is scoped in detail.
5. **`emit_to` retargeting confirmed**, including across the corrected static-window design. The `Mutex<String>` current-target pattern mirroring `run_terminal_bridge`, with the target flipped only after the new window is confirmed ready and before the old one is torn down (and the reverse order on reattach), produced no observed gaps in the `spike-tick` sequence across repeated cycles.
6. **Layout persistence round-trip confirmed**, including the fallback-to-default path — accidentally live-tested when UAT closed every dockview tab via their built-in close button, which persisted the resulting empty layout with no way to recover it. That surfaced a real, separate UX gap worth carrying into Phase 3/4's scoping: **dockview's default tabs are closable, and closing one removes it from the layout entirely with no built-in "reopen a closed panel" affordance** — production needs its own restore mechanism (e.g. a Window-menu "Reveal `<panel>`" action per closed panel, or disabling the close button on panels that must always exist).

**Net recommendation**: proceed to Phase 1 (already independent of this spike) and Phase 2 with dockview confirmed. Before Phase 6 is scoped in detail, update its design to a pre-declared/shown-hidden detached window per finding 4, and make sure Phase 3/4 account for finding 6's "closed panel has no restore path" gap. Findings 1 and 2 de-risk Phase 5 more than expected — no special mitigation needed for the `terminal_ready` timing concern.
...
**Frontend**:
- Add `dockview-react` to `debugger/frontend/package.json` (not `dockview` alone — as of v8, `dockview` is just the framework-agnostic core re-exported from `dockview-core`; the React bindings, including `DockviewReact`, are the separate `dockview-react` package, confirmed in Phase 0's spike. `dockview-react` re-exports everything from `dockview` too, so all types can be imported from the one package.)
...
Restores Terminal's own-window capability, custom-built rather than using dockview's popout. The largest and riskiest phase.

**Revised per Phase 0 findings (finding 4)**: the detached window is a statically declared, initially hidden window (`tauri.conf.json`), shown/hidden on each detach cycle — like Terminal/Trace/Log already are — not dynamically built/destroyed via `WebviewWindowBuilder`. The spike found that building a window with `WebviewWindowBuilder` freezes the entire app (every window's panel updates stop) for the duration of construction, confirmed on every cycle; an A/B test against a statically declared window shown/hidden instead produced no freeze. The bullets below reflect the revised design; anywhere mentioning "build"/"destroy" the window should be read as "show"/"hide" instead.
...
- `tauri.conf.json`: add `terminal-detached` as a statically declared window (fixed label `TERMINAL_DETACHED_WINDOW_LABEL`), initially hidden — same shape as the existing Terminal/Trace/Log entries.
- `terminal.rs`: `detach_terminal(app)` command shows the window and flips a new shared target-label state; `remove_menu()` is called once at startup (mirroring Terminal/Trace/Log's existing `setup()` handling), not per-cycle, since the window is no longer rebuilt.
- New `TerminalTargetWindow(Mutex<String>)` state, read by `run_terminal_bridge` before each `emit_to` call — replaces the hardcoded label with the dynamic target. This is the concrete implementation of the "redirect `emit_to` by panel location" mechanism; unchanged by the static-window revision.
- Closing the detached window: **hide** it — the same hide-not-destroy pattern `install_toggleable_window_lifecycle` already uses for Terminal/Trace/Log, no longer a deliberate divergence — and flip the target label back to `MAIN_WINDOW_LABEL`, emitting an event the main layout listens for to reinsert the Terminal panel (which still mounts a fresh `TerminalPanel` instance, since the dock model still removes/re-adds the panel component regardless of the window's own show/hide lifecycle).
...
**Headline risks**:
1. **`emit_to`-retarget race**: a PTY-output chunk could theoretically be read between the old target tearing down and the new one existing. Phase 0's A/B test found this race is much narrower with a shown/hidden static window than the originally planned dynamic build (which guaranteed an app-wide freeze on every cycle; the static path only occasionally showed a related orphaned-callback warning), but mitigate the same way regardless: flip the target-label state only after the new window/panel is fully constructed and before tearing down the old one; accept a narrow, low-consequence window where a byte or two could be lost mid-detach (comparable to a terminal multiplexer detach). UAT case: type continuously while clicking Detach, confirm no dropped/garbled output.
...
**Validation checklist**: detach → type → verify no drop → reattach, repeated 3+ times (show/hide correctness across repeated cycles, not just once); close via native chrome vs. in-app action; clipboard in the detached window; theme sync in the detached window; exit-while-detached.
