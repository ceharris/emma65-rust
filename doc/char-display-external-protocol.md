...
Exactly one connection, outbound (device → peripheral) only — the device consumes no inbound
data over this transport. The transport's [`Transport::send_bytes`] must be atomic: either the
whole buffer is written or none of it is (see the transport module's documentation and the SDL2
display peripheral plan's Unit 1). This rules out any transport whose `send_bytes` is the
default per-byte-loop fallback; in practice `PipeTransport` is the only supported
implementation, and the config module (Unit 3 of the SDL2 display peripheral plan) enforces this
by rejecting any other transport spec at instantiate time.
...
| `palette_len`   | `u16` LE  | 2            | fixed for the connection's lifetime (see §6)        |
...
No inbound messages, no reconnection support (the design assumes a single spawned child process
tied to the device's lifetime — see Unit 4 of the SDL2 display peripheral plan), no protocol
negotiation. A version mismatch has no defined recovery — a peripheral that doesn't recognize a
